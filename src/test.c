...
{
	printf("INPUT STRUCT\n");
	printf("%d\n", I.x_assemblies);          	/* Number of assemblies in the x-axis of the 
								   reactor */
	printf("%d\n", I.y_assemblies);           /* Number of assemblies in the y-axis of the 
								   reactor */
	printf("%d\n", I.cai);                    // Number of course axial intervals
	printf("%d\n", I.fai);                    /* Number of fine axial intervals per coarse 
								   axial interval */
	...
	printf("%d\n", I.decomp_assemblies_ax);  	// Number of assemblies per sub-domain (axially)
	printf("%ld\n", I.segments_per_track);   	// Average number of segments per track
	printf("%lf\n", I.assembly_width);     	// Width of an assembly - 1.26 x 17 cm
